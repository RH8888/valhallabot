...
def ensure_schema() -> None:
    ...
    with with_mysql_cursor() as cur:
        ...
        try:
            cur.execute(
                "ALTER TABLE panels ADD COLUMN append_ratio_to_name TINYINT(1) NOT NULL DEFAULT 0 AFTER usage_multiplier"
            )
        except MySQLError:
            pass
        try:
        ...
        try:
            cur.execute(
                "ALTER TABLE panels ADD COLUMN panel_api_version VARCHAR(8) NULL AFTER admin_password_encrypted"
            )
        except MySQLError:
            pass
        try:
