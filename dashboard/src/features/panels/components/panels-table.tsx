...
import { panelsColumns as columns } from './panels-columns'
...
export function PanelsTable({ data, search, navigate, isLoading = false }: PanelsTableProps) {
  ...
  const typeFilterOptions = useMemo(
    () => PANEL_TYPES.map((type) => ({ label: type.label, value: type.value })),
    []
  )
  ...
  return (
    ...
    >
      <DataTableToolbar
        table={table}
        searchPlaceholder='Filter panels by name...'
        searchKey='name'
        filters={[{ columnId: 'panel_type', title: 'Type', options: typeFilterOptions }]}
      />
            ...
            {isLoading ? (
              <TableRow>
                <TableCell colSpan={columns.length} className='h-24 text-center'>
                  <LoadingOverlay label='Loading panels…' />
                </TableCell>
              </TableRow>
