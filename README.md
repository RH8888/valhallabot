...
is running. By default the dashboard pulls the published container image from
`ghcr.io/rh8888/valhallabot-dashboard`. If you maintain a custom dashboard
build—such as one with a different `DASHBOARD_BASE_URL` or
`DASHBOARD_API_BASE_URL` baked in—build an image from [`dashboard/`](./dashboard)
and set `DASHBOARD_IMAGE` in `/app/.env` to point at your tag. The
`setup.sh` script writes both
...
For Podman replace `docker compose` with `podman compose -f podman-compose.yml`.
The `.env` file keeps all of your answers so rerunning the setup script is not
...
| `DASHBOARD_PORT`       | Host port that exposes the production dashboard container.              |
| `DASHBOARD_INTERNAL_PORT` | Container port bound by the Node-based dashboard server (default `4173`). |
| `DASHBOARD_BASE_URL`   | Base path (e.g. `/` or `/dashboard`) used when building static assets.  |
| `DASHBOARD_API_BASE_URL` | API endpoint embedded into the dashboard build (defaults to `http://app:<FLASK_PORT>/api/v1`). |
| `DASHBOARD_IMAGE`      | Dashboard container image tag (defaults to `ghcr.io/rh8888/valhallabot-dashboard:v1.0.0`). |
